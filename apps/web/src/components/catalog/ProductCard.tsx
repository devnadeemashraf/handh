...
export function ProductCard({
  ...
  const badgeConfig = React.useMemo(() => {
    ...
    return {
      label: 'In Stock',
      variant: 'default' as const,
      className: 'bg-surface/90 text-royal border-border-subtle shadow-xs'
    };
  ...
  return (
    <>
      <article
        className={cn(
          'group relative flex flex-col overflow-hidden rounded-md bg-surface border border-border-subtle transition-all duration-base hover:border-border-strong',
          className
        )}
      >
        ...
        <div className="relative aspect-[4/5] w-full overflow-hidden bg-sunken rounded-t-md">
          <Link
          ...
          >
            {product.primaryImageUrl ? (
              ...
              <div className="flex h-full w-full items-center justify-center p-4 text-center font-serif text-xs tracking-widest uppercase text-text-tertiary">
          ...
          {product.isAvailable && (
            <div className="absolute bottom-2 right-2 z-10 md:hidden">
              <button
                ...
                className="flex h-11 w-11 items-center justify-center rounded-sm bg-surface/95 text-text-primary shadow-elevation-1 border border-border-subtle active:scale-95 transition-transform"
              >
          ...
          {product.isAvailable && (
            <div className="absolute inset-x-2.5 bottom-2.5 hidden z-10 md:block transition-all duration-base translate-y-full opacity-0 group-hover:translate-y-0 group-hover:opacity-100">
              <Button
                ...
                onClick={handleQuickAddClick}
                disabled={isDirectAdding}
                className="w-full h-9 bg-surface/95 backdrop-blur-sm text-text-primary hover:bg-surface border-border-strong text-[11px] uppercase tracking-wider font-medium shadow-elevation-1 transition-colors"
              >
        ...
        <div className="flex flex-1 flex-col justify-between p-3 sm:p-4">
          <div>
            {product.categoryName && (
              <span className="mb-1 block text-[10px] uppercase tracking-[0.18em] font-medium text-text-secondary truncate">
                {product.categoryName}
              </span>
            )}

            {/* Single-line title with ellipsis truncation */}
            <h3 className="font-sans text-xs sm:text-sm font-medium text-text-primary group-hover:text-royal transition-colors truncate">
              <Link
                href={`/products/${product.slug}`}
            ...
            {product.colorCount && product.colorCount > 1 ? (
              <p className="mt-1 text-[11px] text-text-secondary">{product.colorCount} colors</p>
            ) : null}
          </div>

          {/* Tabular Price Row */}
          <div className="mt-3 flex items-baseline justify-between border-t border-border-subtle pt-2.5">
            <div className="flex items-baseline gap-2">
              <span
                className={cn(
                  'font-mono text-xs sm:text-sm font-medium tabular-nums',
                  comparePrice ? 'text-sale' : 'text-text-primary'
                )}
              >
                {formattedPrice}
              </span>
              {comparePrice && (
                <span className="font-mono text-[11px] tabular-nums text-text-tertiary line-through">
                  {comparePrice}
                </span>
              )}
            </div>
...
export function ProductCardSkeleton() {
  return (
    <div className="flex flex-col overflow-hidden rounded-md border border-border-subtle bg-surface">
      {/* 4:5 image skeleton */}
      <Skeleton className="aspect-[4/5] w-full rounded-t-md rounded-b-none bg-sunken" />

      {/* Content skeleton */}
      <div className="space-y-2 p-3 sm:p-4">
        <Skeleton className="h-2.5 w-1/3 bg-sunken" />
        <Skeleton className="h-4 w-4/5 bg-sunken" />
        <div className="mt-3 flex items-center justify-between border-t border-border-subtle pt-2.5">
          <Skeleton className="h-3.5 w-16 bg-sunken" />
          <Skeleton className="h-2.5 w-12 bg-sunken" />
        </div>
      </div>
    </div>
  );
