...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      'fixed inset-0 z-50 bg-black/40 backdrop-blur-sm transition-opacity duration-base data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 select-none',
      className
    )}
    {...props}
  />
  ...
  <ModalSheetPortal>
    ...
    <DialogPrimitive.Content
      ...
      className={cn(
        ...
        'fixed inset-x-0 bottom-0 z-50 max-h-[90vh] overflow-y-auto rounded-t-md bg-card p-6 shadow-lg border-t border-border transition-all ease-decelerate duration-base data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom',
        // Desktop / Tablet (>=768px): Centered Modal
        'md:inset-auto md:left-[50%] md:top-[50%] md:bottom-auto md:max-w-lg md:w-full md:max-h-[85vh] md:translate-x-[-50%] md:translate-y-[-50%] md:rounded-md md:border md:border-border md:slide-in-from-bottom-0 md:zoom-in-95 data-[state=closed]:zoom-out-95',
        className
      )}
      {...props}
    >
      {/* Mobile drag handle */}
      <div
        className="mx-auto -mt-2 mb-4 h-1 w-8 rounded-full bg-muted-foreground/30 md:hidden"
        aria-hidden="true"
      />
      {children}
      {showCloseButton && (
...
const ModalSheetFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn(
      'flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 pt-4 mt-4 border-t border-border',
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={cn('font-serif text-lg font-medium text-foreground leading-none', className)}
    {...props}
  />
));
